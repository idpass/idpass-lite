...
void single_instance_test(void* ctx)
{
    ...
    ASSERT_TRUE(cards.publiccard().details().givenname().compare("Manny") == 0);
    ASSERT_TRUE(cards.publiccard().details().dateofbirth().ByteSizeLong() == 0);
    ASSERT_TRUE(cards.publiccard().details().placeofbirth().compare("Kibawe, Bukidnon") == 0);
...
#if 1
    void* context = idpass_lite_init(keysetbuf.data(), keysetbuf.size(), 
