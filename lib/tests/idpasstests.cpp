...
unsigned char encryptionKey[] = {
  0xfa, 0xce, 0xde, 0xc0, 0xde, 0xd1, 0xe8, 0x60, 
...
TEST_F(idpass_api_tests, check_skpk_validity2)
{
    unsigned char invalid_ed25519_key[] = {
        0x22, 0x52, 0xf8, 0x6a, 0xaa, 0x4d, 0x62, 0xfc, 
        0xab, 0x4d, 0xb0, 0x0a, 0x21, 0x1a, 0x12, 0x60, 
        0xf8, 0x17, 0xc5, 0xf2, 0xba, 0xb7, 0x3e, 0xfe, 
        0xd6, 0x36, 0x07, 0xbc, 0x9d, 0xb3, 0x96, 0xee, 
        0x57, 0xc6, 0x33, 0x09, 0xfa, 0xc2, 0x1b, 0x60, 
        0x04, 0x76, 0x4e, 0xf6, 0xf7, 0xc6, 0x2f, 0x28, 
        0xcf, 0x63, 0x40, 0xbe, 0x13, 0x10, 0x6e, 0x80, 
        0xed, 0x70, 0x41, 0x8f, 0xa1, 0xb9, 0x27, 0xb4}; // 64

    void* context = idpass_api_init(encryptionKey,
                          crypto_aead_chacha20poly1305_IETF_KEYBYTES,
                          invalid_ed25519_key,
                          crypto_sign_SECRETKEYBYTES,
                          verification_pk,
...
TEST_F(idpass_api_tests, check_skpk_validity)
{
    unsigned char invalid_ed25519_key[] = {
        0x22, 0x52, 0xf8, 0x6a, 0xaa, 0x4d, 0x62, 0xfc, 
        0xab, 0x4d, 0xb0, 0x0a, 0x21, 0x1a, 0x12, 0x60, 
    ...
    ASSERT_TRUE(0 == crypto_sign_ed25519_sk_to_pk(pubkey, invalid_ed25519_key));
    ...
    status = crypto_sign_detached(
        signature, nullptr, reinterpret_cast<const unsigned char*>(msg), 
        std::strlen(msg), invalid_ed25519_key);
    ASSERT_TRUE(status == 0);
    status = crypto_sign_verify_detached(
        signature, reinterpret_cast<const unsigned char*>(msg), 
        std::strlen(msg), pubkey);
    ASSERT_TRUE(status != 0);
}
...
TEST_F(idpass_api_tests, generate_secretsignature_key)
{
    unsigned char sig[crypto_sign_SECRETKEYBYTES]; // 64
    unsigned char sig2[63];
    int status;

    status = idpass_api_generate_secret_signature_key(sig, sizeof sig);
    ASSERT_TRUE(status == 0);
    status = idpass_api_generate_secret_signature_key(sig2, sizeof sig2);
    ASSERT_TRUE(status != 0);
}

TEST_F(idpass_api_tests, generate_encryption_key)
{
    unsigned char enc[crypto_aead_chacha20poly1305_IETF_KEYBYTES]; // 32
    unsigned char enc2[31];
    int status;

    status = idpass_api_generate_encryption_key(enc, sizeof enc);
    ASSERT_TRUE(status == 0);
    status = idpass_api_generate_encryption_key(enc2, sizeof enc2);
    ASSERT_TRUE(status != 0);
}
