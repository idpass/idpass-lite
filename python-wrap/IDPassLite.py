from ctypes import *
import sys
from IDPassLoader import IDPassLoader
...
class Helper(object):
    def generate_encryption_key():
        key = (c_ubyte * 32)()
        IDPassLoader.lib.idpass_lite_generate_encryption_key(key, len(key))
        return key

    def generate_secret_signature_key():
        key = (c_ubyte * 64)()
        IDPassLoader.lib.idpass_lite_generate_secret_signature_keypair(key, len(key))
        return key
...
class Reader(object):
    ...
    encryptionkey = None
    signaturekey = None
    verificationkeys = None

    def __init__(self, enc, sig, verif):
        self.encryptionkey = enc
        self.signaturekey = sig
        self.verificationkeys = verif

        keyset = api_pb2.KeySet()
        keyset.encryptionKey = self.encryptionkey
        keyset.signatureKey = self.signaturekey
        keyset.verificationKeys = self.verificationkeys
        keysetba = bytearray(keyset.SerializeToString())

        self.ctx = IDPassLoader.lib.idpass_lite_init((ctypes.c_ubyte * len(keysetba))(*keysetba), len(keysetba), None , 0)
    ...
    def ioctl(self, cmd, param):
        ...
        ioctlcmd = [ cmd, param ] 
        IDPassLoader.lib.idpass_lite_ioctl( self.ctx, None, (c_ubyte * len(ioctlcmd))(*ioctlcmd), len(ioctlcmd) )
    ...
    def compare_face_template(self, input1, input2):
        fdiff = c_float(10.0)
        status = IDPassLoader.lib.idpass_lite_compare_face_template(
    ...
    def computeFullTemplate(self, inputfile):
        ...
        fdim = (c_ubyte *512)()
        facecount = IDPassLoader.lib.idpass_lite_face128dbuf( 
                        self.ctx,
    ...
    def computeHalfTemplate(self, inputfile):
        ...
        hdim = (c_ubyte *128)()
        facecount = IDPassLoader.lib.idpass_lite_face64dbuf(
                        self.ctx,
                        (c_ubyte * len(photo))(*photo), 
                        len(photo), 
                        hdim
    ...
    def computeFullTemplateAsFloats(self, inputfile):
        photo = open(inputfile, "rb").read()
        fdim = (c_float *128)()
        facecount = IDPassLoader.lib.idpass_lite_face128d(
                        self.ctx,
                        (c_ubyte * len(photo))(*photo), 
                        len(photo), 
                        fdim)
    ...
    def compare_face_photo(self, inputfile1, inputfile2):
        ...
        fdiff = c_float(10.0)
        status = IDPassLoader.lib.idpass_lite_compare_face_photo(
            self.ctx,
    ...
    def computeHalfTemplateAsFloats(self, inputfile):
        photo = open(inputfile, "rb").read()
        hdim = (c_float *64)()
        facecount = IDPassLoader.lib.idpass_lite_face64d(
                        self.ctx,
                        (c_ubyte * len(photo))(*photo), 
                        len(photo), 
                        hdim)
    ...
    def create_card_with_face(self, inputfile):
        ...
        buf = IDPassLoader.lib.idpass_lite_create_card_with_face(
            self.ctx, 
            byref(buf_len),
            surname, 
            givenname, 
    ...
    def asQRCode(self, data):
        ...
        buf = IDPassLoader.lib.idpass_lite_qrpixel2(self.ctx, byref(buf_len), input, data_len, byref(qr_side_len))
    ...
    def freemem(self, addr):
        IDPassLoader.lib.idpass_lite_freemem(self.ctx, addr)
